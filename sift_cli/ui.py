...
def build_sidebar_text(
    state: SearchState, *, roots: tuple[Path, ...] = (), has_index: bool = False
) -> str:
...
def build_result_row_text(
    ...
    *,
    ...
    ordinal = (
        f"[{index + 1}/{total}] " if index is not None and total is not None else ""
    )
    badges: list[str] = []
...
def build_results_text(
    ...
    *,
    ...
    if state.indexing and not index_ready:
        return "Building the first index... results will appear after the first build."

    if (
        state.indexing
        and index_ready
        and not state.raw_query.strip()
        and not state.results
    ):
        return "Indexing... search stays available against the last completed index."

    if state.loading:
        if index_ready:
            return (
                "Indexing... search stays available against the last completed index."
            )
        return "Building the first index... results will appear after the first build."
    ...
    if not state.results:
        ...
        if summary:
            return (
                f"No matching files for: {state.raw_query}\nActive filters: {summary}"
            )
...
def build_status_text(
    ...
    *,
    ...
    if state.indexing:
        ...
        return "Indexing in progress. The first index is not ready yet."
    if state.loading:
        return "Searching..."
    if not state.raw_query.strip():
        if not has_index and not state.has_index:
            return (
                f"Configured roots: {', '.join(str(root) for root in roots) or 'none'}"
            )
...
class SearchController:
    def __init__(
        self, *, fuzzy_index: FuzzyIndex | None = None, db_path: Path | None = None
    ) -> None:
    ...
    def begin_search(self, query: str) -> SearchRequest:
        ...
        self.state = replace(
            self.state, raw_query=query, loading=True, focus_mode="input"
        )
    ...
    def complete_search(
        self, request: SearchRequest | int, results: Sequence[SearchResult]
    ) -> None:
        request_id = (
            request.request_id if isinstance(request, SearchRequest) else request
        )
        if request_id != self._active_request_id:
            return
        previous_selected_path = (
            self.active_result.path if self.active_result is not None else None
        )
    ...
    def update_query(self, query: str, *, cursor: int | None = None) -> None:
        ...
        try:
            ...
            parsed_query = None
        autocomplete = (
            tuple(autocomplete_suggestions(query, self._fuzzy_index, cursor=cursor))
            if query.strip()
            else ()
        )
        mode = (
            "empty"
            if not query.strip()
            else ("autocomplete" if autocomplete else "ready")
        )
    ...
    def search(self, query: str) -> list[SearchResult]:
        ...
        if not query.strip():
            self.state = replace(
                self.state, results=(), loading=False, selected_index=0
            )
            return []
        if self._db_path is None:
            self.state = replace(
                ...
                loading=False,
                mode="no-results",
                status_message="No index database configured.",
            )
            return []
        ...
        try:
            ...
            self.state = replace(
                self.state, loading=False, status_message=f"Query error: {exc}"
            )
            return []
        except Exception as exc:
            self.state = replace(
                self.state, loading=False, status_message=f"Search error: {exc}"
            )
            return []
    ...
    def move_result_selection(self, delta: int) -> None:
        if not self.state.results:
            return
        next_index = max(
            0, min(len(self.state.results) - 1, self.state.selected_index + delta)
        )
        self.state = replace(
            self.state, selected_index=next_index, focus_mode="results"
        )
    ...
    def move_autocomplete_selection(self, delta: int) -> None:
        ...
        next_index = max(
            0,
            min(
                len(self.state.autocomplete) - 1, self.state.autocomplete_index + delta
            ),
        )
        self.state = replace(
            self.state, autocomplete_index=next_index, focus_mode="autocomplete"
        )

    def accept_autocomplete(self) -> str:
        return self.accept_autocomplete_with_cursor()[0]

    def accept_autocomplete_with_cursor(
        self, cursor: int | None = None
    ) -> tuple[str, int]:
        if not self.state.autocomplete:
            current_cursor = (
                len(self.state.raw_query)
                if cursor is None
                else max(0, min(cursor, len(self.state.raw_query)))
            )
    ...
    def set_query_error(self, message: str) -> None:
        self.state = replace(
            self.state, loading=False, status_message=f"Query error: {message}"
        )

    def set_search_error(self, message: str) -> None:
        self.state = replace(
            self.state, loading=False, status_message=f"Search error: {message}"
        )

    def set_indexing_error(self, message: str) -> None:
        self.state = replace(
            self.state, indexing=False, status_message=f"Indexing failed: {message}"
        )

    def set_indexing_success(
        self,
        *,
        files_indexed: int,
        files_skipped: int = 0,
    ...
    def set_indexing_already_running(self) -> None:
        self.state = replace(
            self.state, status_message="Index refresh already running."
        )
    ...
    def open_selected_result(self) -> None:
        ...
        try:
            ...
            self.state = replace(
                self.state, status_message=f"Missing file: {result.path}"
            )
            return
            ...
            self.state = replace(
                self.state, status_message=f"Could not open file: {exc}"
            )
            return
